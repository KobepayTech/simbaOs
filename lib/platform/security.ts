...
export async function signIn(db:D1Database,phone:string){await db.prepare('UPDATE profiles SET verified_phone=? WHERE member_id IN (SELECT id FROM members WHERE user_id=? AND phone=?)').bind(phone,'phone:'+phone,phone).run();const token=crypto.randomUUID()+crypto.randomUUID(),expires=new Date(Date.now()+30*86400000).toISOString();await db.prepare('INSERT INTO sessions VALUES(?,?,?,?)').bind(await hash(token),'phone:'+phone,phone,expires).run();(await cookies()).set('simba_session',token,{httpOnly:true,secure:true,sameSite:'lax',path:'/',maxAge:30*86400});}
